...
页面优先呈现品牌、搜索、分类入口、资料概览、目录导航和资源卡片。Google Drive 可以作为资料源接入，但不是模板的默认主题；没有配置真实资料源时，页面会以“资料源待配置”和通用示例资料展示。

## 核心文件

- `index.html`：资料书架页面骨架，包含品牌区、搜索区、概览区、导航侧栏、资源列表和预览弹窗容器。
- `style.css`：Lybris 资料书架视觉样式，包括玻璃面板、资源卡片、响应式布局和预览样式。
- `script.js`：加载配置与 `data/drive-index.json`，渲染分类入口、目录树、资料卡片、搜索、概览统计、面包屑和资料预览。
...
- `driveFolderUrl` 默认为空，顶部按钮显示“资料源待配置”。
...
- `decorativeChips`：低权重背景装饰文案。建议使用通用词，不要把某一学科的公式、符号或术语写入模板核心。
...
然后打开 `http://localhost:8000/`，确认页面标题、学科名、分类入口、资料概览、资料列表、搜索、文件夹导航、资料源链接和可预览资料都能正常工作。
